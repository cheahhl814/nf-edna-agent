...
The merged parameters JSON was generated by combining `runners/nextflow-runner/params/12s.json` (marker preset, since the dataset is MiFish-U / 12S rRNA — see "Marker-detection correction" above) + `run/params.json` (run-specific overrides, last wins):

```python
merged = {**preset, **run_specific}  # preset = runners/nextflow-runner/params/12s.json (MiFish-U / 12S rRNA preset)
```
...
**Fix**: removed `--maximum-length` from `runners/nextflow-runner/modules/qc.nf`. NGmerge (denoise/merge_pairend) enforces length filtering post-merge. Also raised `runners/nextflow-runner/params/12s.json.max_length` from 200 to 220 as a safety margin. Updated `runners/nextflow-runner/modules/qc.nf` to document this in a comment.
...
```bash
nextflow run . -params-file runners/nextflow-runner/params/16s.json -params-file params.json
```
...
> | `Can only specify option -params-file once` | Nextflow ≥ 24 rejects multiple `-params-file` flags (the v1.0.0 body instructs this incorrectly) | Merge `runners/nextflow-runner/params/{marker}.json` + `run/params.json` into one JSON object before `-params-file <merged>.json` |
...
- **Marker correction**: v1.0.0 said "16S V3-V4"; corrected to **12S / MiFish-U** after user pointed out the actual primers. All intake-state files regenerated against `runners/nextflow-runner/params/12s.json`.
...
python3 -c "
import json
preset = json.load(open('runners/nextflow-runner/params/16s.json'))
