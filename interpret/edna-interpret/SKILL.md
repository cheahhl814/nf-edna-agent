...
- **Blank contamination**: check `blank_qc` in `run_summary.json`. If the list is non-empty, a blank sample has more reads than one or more biological samples — flag this prominently. Decontam may not catch reagent contaminants that co-occur in biological samples. Also check `asv_counts.contaminants_flagged` — if 0 despite contaminated blanks, flag for human review. **Important**: both negative controls in this run are shared across all biological samples (see `pipeline_state.json.shared_negatives` if present, or the design-decision in `preflight/edna-intake` evidence E4); contamination findings apply to the run as a whole, not a specific site.
